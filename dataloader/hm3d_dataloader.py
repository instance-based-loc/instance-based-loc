...
class HM3Dataloader(BaseDataLoader):
    def __init__(
            self, 
            data_path: str, 
        ...
        # Set poses
        self._poses = np.load(self._pose_information_file_path)
        pose_file_data = np.load(self._pose_information_file_path)
        ...
        #     q = Rotation.from_matrix(R1).as_quat()
    ...
    def setup_map_pointcloud(self) -> None:
        ...
        for env_idx in tqdm([i for i in self.environment_indices][::50], desc="Forming pointcloud map from env. images"):
            ...
            depth_img = np.load(self._depth_images_paths[env_idx]).squeeze()
            # depth_img /= 5000.0 # for kinect cameraqqd
            cur_pointcloud = \
