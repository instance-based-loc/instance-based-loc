...
class AttentionWithLoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.qkv_lora_down_matrix = nn.Parameter(torch.randn(dim, 4))  
        self.qkv_lora_up_matrix = nn.Parameter(torch.zeros(4, 3 * dim))  
        self.attn_drop = nn.Dropout(attn_drop) 
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
    ...
    def forward(self, x):
        B, N, C = x.shape
        lora_weight = self.qkv_lora_down_matrix @ self.qkv_lora_up_matrix 
        qkv = (self.qkv(x) + F.linear(x, lora_weight.T)).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, lora=False):
        super().__init__()
...
class PatchEmbed_overlap(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768):
        ...
        # making this change because img_size = ([256, 128], [256, 128])
        img_size = img_size[0]
        stride_size_tuple = stride_size_tuple[0]
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, lora=False)
            for i in range(depth-2)] + [
            Block(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, lora=True) 
            for i in range(2)]  
