...
class build_FourDNet(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory, gpu0, gpu1, target_gpu, rearrange):
        ...
        self.rgb_pretrained_path = "./logs/FourDNet_RGB_attempt1/100.pth"
        self.depth_pretrained_path = "./logs/FourDNet_Depth_attempt3/100.pth"
        ...
        if pretrain_choice == 'imagenet':
            ...
            for n, p in self.base.named_parameters(): 
                if "qkv_lora_down_matrix" in n: 
                    continue 
                p.requires_grad = False  
            for n, p in self.base2.named_parameters(): 
                if "qkv_lora_up_matrix" in n: 
                    continue 
                p.requires_grad = False 
    ...
    def load_rgb_pretrained(self):
        ...
        for i in param_dict:
            ...
            # elif i.find("base") != -1:
            #     self.state_dict()[i].copy_(param_dict[i])
            if i.find("rgb") != -1 or i.find("r2r") != -1 or i.find("Q_r") != -1 or i.find("V_r") != -1: 
                self.state_dict()[i].copy_(param_dict[i])
    ...
    def load_depth_pretrained(self):
        ...
        for i in param_dict:
            # if i.find("base2") != -1:
            #     self.state_dict()[i].copy_(param_dict[i])
            if i.find("depth") != -1 or i.find("d2d") != -1 or i.find("Q_d") != -1 or i.find("V_d") != -1:
                self.state_dict()[i].copy_(param_dict[i])

    def load_param(self, trained_path, load_classifier=False):
        param_dict = torch.load(trained_path)
        for i in param_dict:
            if i.find("classifier") != -1 and not load_classifier: 
                continue 
            self.state_dict()[i.replace('module.', '')].copy_(param_dict[i])
        print('Loading pretrained model from {}'.format(trained_path))
    ...
    def forward(self, rgb, depth, label=None, cam_label= None, view_label=None):  # label is unused if self.cos_layer == 'no'
        ...
        # assert torch.allclose(rgb_filter.to(self.gpu1) + depth_filter, torch.ones(B, 16, 8).to(self.gpu1)), f"diff = {torch.sum(rgb_filter.to(self.gpu1) + depth_filter - torch.ones(B, 16, 8)).to(self.gpu1)}"  
        ...
        # adding back to the depth path
        local_cat_global_rgb = local_cat_global_rgb + d2r_feat.to(self.gpu1) * rgb_filter.reshape(B, 128).unsqueeze(-1).to(self.gpu1)
        local_cat_global_rgb = self.d2r_norm(local_cat_global_rgb)
        ...
        # adding back to the RGB path
        local_cat_global_depth = local_cat_global_depth + r2d_feat.to(self.gpu0) * depth_filter.reshape(B, 128).unsqueeze(-1).to(self.gpu0)
        local_cat_global_depth = self.r2d_norm(local_cat_global_depth) 
        ...
        # final_embedding = global_rgb.to(self.target_gpu) 
