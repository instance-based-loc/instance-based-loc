...
# Process all datasets except the first two for cuda:2
...
do
    ...
    echo "Processing ${dataset_clip} on cuda:2..."
    CUDA_VISIBLE_DEVICES=2 python tum_localisation_trial.py -t ${dataset_clip} --data-path "${data_path}synced_data/" --map-pcd-cache-path "${map_pcd_cache_path}" --memory-load-path "${memory_load_path}" --embeddings clip > "${log_file}"
done

echo "All datasets processed on cuda:2."
