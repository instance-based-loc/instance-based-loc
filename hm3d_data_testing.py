...
for i in tqdm(range(len(pose_data))[18:30]):
# for i in tqdm(range(len(pose_data))[36:41]):
    ...
    cur_pcd = depth_utils.get_coloured_pointcloud_from_depth(
        depth_image,
        rgb_image,
        fx,
        fy,
    ...
    pose_updated = pose_data[i]
    pose_updated[-2] *= -1

    transformed_pointcloud = depth_utils.transform_pointcloud(
        cur_pcd,
        pose_updated
    )

    full_point_cloud += transformed_pointcloud
